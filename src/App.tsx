...
function App() {
  ...
  const syncQueue = async () => {
    ...
    for (const queued of queuedTransactions) {
      try {
        ...
        const response = await fetch(`${API_BASE_URL}/api/transactions`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...transactionPayload, transaction_type: type }) })
  ...
  const addTransaction = async (event: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/transactions`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
  ...
  const deleteTransaction = async (transaction: Transaction) => {
    try { await fetch(`${API_BASE_URL}/api/transactions/${transaction.id}`, { method: 'DELETE' }) } catch { /* local retention still applies */ }
  ...
  const startRecording = async () => {
    ...
    recorder.onstop = async () => {
      ...
      try {
        const response = await fetch(`${API_BASE_URL}/api/transcribe`, { method: 'POST', body: formData }); const result = (await response.json()) as { transcript?: string; detail?: string }
